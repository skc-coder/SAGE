---
exam: "CDS"
subject: "Elementary Mathematics"
topic: "Number System"
difficulty: "Medium"
tags: [cds, elementary-mathematics, number-system, variations]
---

# Number System Variations

## Tier 1 & Tier 2 Variations

### Variation 1
If $\frac{x}{3} = \frac{y}{7} = \frac{z}{11} = \frac{w}{15}$, find the value of $\frac{2x + 3y + z}{w - y}$.

- **User Answer**: (b) $4.75$ — **CORRECT!**

> [!faq]- View Solution
> Direct substitution: $x=3, y=7, z=11, w=15$.
> $$\frac{2(3) + 3(7) + 11}{15 - 7} = \frac{6 + 21 + 11}{8} = \frac{38}{8} = 4.75$$

---

### Variation 2
If $\frac{a}{2} = \frac{b}{3} = \frac{c}{5}$, then the value of $\frac{3a - 2b + 4c}{3(2) - 2(3) + 4(5)}$ is equal to:

- **User Answer**: (a) $\frac{a}{2}$ — **CORRECT!** (Note: Both (a) $\frac{a}{2}$ and (d) $\frac{a+b+c}{20}$ are mathematically equal to $k$ by the Addendo property).

> [!faq]- View Solution
> By Weighted Addendo Property:
> $$\frac{3a - 2b + 4c}{3(2) - 2(3) + 4(5)} = \frac{a}{2} = \frac{b}{3} = \frac{c}{5} = k$$
> Also, by standard Addendo: $\frac{a+b+c}{2+3+5} = \frac{a+b+c}{10}$, or $\frac{2(a+b+c)}{20}$. Thus $\frac{a}{2}$ is direct.

---

### Variation 3
If $\frac{p}{3} = \frac{q}{4} = \frac{r}{5}$, calculate the value of $\frac{p^2 + q^2 + r^2}{pq + qr}$.

- **User Answer**: (c) $\frac{50}{32} = \frac{25}{16}$ — **CORRECT!** (Note: $\frac{50}{32}$ simplifies to $\frac{25}{16}$, which matches option (b) in simplified form).

> [!faq]- View Solution
> Direct substitution: $p=3, q=4, r=5$.
> $$\frac{3^2 + 4^2 + 5^2}{(3)(4) + (4)(5)} = \frac{9 + 16 + 25}{12 + 20} = \frac{50}{32} = \frac{25}{16}$$

## Navigation

- [[content/cds/elementary-mathematics/elementary_mathematics_overview|Elementary Mathematics Overview]]
